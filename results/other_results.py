...
# Read the dataset
results_json = "../cross_domains_cross_models/jsons/other_overall.json"
dataset = sys.argv[1]
single_features = ["fastdetect", "t5sentinel"]
multi_features = []
...
with open(results_json, 'r') as file:
    ...
    random.shuffle(results_test)
    results_train.extend(results_test[:int(len(results_test) * 0.8)])
    results_test = results_test[int(len(results_test) * 0.8):]
